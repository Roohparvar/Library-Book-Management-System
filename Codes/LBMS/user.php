...
<?php
    ...
    if (isset($_GET['username'])) {
        ...
        $validate_sql = "SELECT * FROM Users WHERE User = '$username'";
        ...
        if ($validate_result->num_rows > 0) {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST['return'])) {
                    ...
                    if ($borrowed_books_count < 3) {
                        ...
                        $insert_sql = "INSERT INTO $borrow_table (User, ISBN, BorrowDate) VALUES ('$username', '$isbn_to_borrow', '$borrow_date')";
                            if ($conn->query($insert_sql) === TRUE) {
                                echo "<p>Book with ISBN $isbn_to_borrow borrowed successfully on $borrow_date.</p>";
                            } else {
            ...
            $sql = "SELECT b.Name, b.Author, b.ISBN, br.BorrowDate 
                    FROM $books_table b
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<ul>";
            while ($row = $result->fetch_assoc()) {
                $borrow_date = date('Y-m-d H:i:s', strtotime($row['BorrowDate']));
                echo "<li><i class='fas fa-book-reader'></i> " . $row['Name'] . " by " . $row['Author'] . " (ISBN: " . $row['ISBN'] . ")";
                echo " - Borrowed on: " . $borrow_date;
